...
ddir='data/'
region_files=['sas50_region.nc']
region_cols=['BlueViolet']
...
def draw_box( coords, m, col,label):
    lat = np.linspace(coords[0], coords[1])
    lon = np.linspace(coords[2], coords[3])
    for i in range(0,4):
               if i==0:
    ...
    plt.text(((82.+100.)/2.)/((coords[2]+coords[3])/2.),((18.+32.)/2.)/((coords[0]+coords[1])/2.),label)

def draw_mask( mask,glat,glon, m, col):
    mask_data=cdo.selvar('region_mask', input=ddir+mask,returnCdf=True).variables['region_mask'][:]
    x,y=m(glon[:],glat[:])
    m.contourf(x,y,mask_data,colors=col,alpha=0.6)
...
for ir,region_file in enumerate(region_files):
	...
	if region_file=="afr50_region.nc" or region_file=="dub25_region.nc" or region_file=="cafr25_region.nc":
	    ...
	    m = Basemap(llcrnrlon=82,llcrnrlat=18,urcrnrlon=100,urcrnrlat=32,\
                               rsphere=(6378137.00,6356752.3142),\
                        resolution='l',area_thresh=1000.,projection='lcc',\
                        lat_1=(18+32)/2,lon_0=(82+100)/2)
	    ...
	    draw_mask("mask_Brahmaputra_nan.nc",glat,glon,m,"Red")

	    #Add in the Ruksana boxes
	    draw_box([24,26,88,90], m, "White" ,"1")
            draw_box([24,25.5,90.5,92.5], m, "White","2" )
	    draw_box([21.5,23.5,89,91], m, "White","3" )
            draw_box([20.5,24,91,93], m, "White","4" )

plt.tight_layout()
fig.savefig("region_plot_sas50_Brahmaputra_box.png")
